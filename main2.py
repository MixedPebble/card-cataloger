...
def run():
    ...
    # Create background subtractor for motion detection. This seperates the moving objects (foreground) from the stationary objects (background).
    background_seperator = cv2.createBackgroundSubtractorMOG2()
    while True:
        # Read a frame from the video capture
        is_frame_read_success, frame = cap.read()
